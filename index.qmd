...
\\ \underline{V} &= \text{diag}([\kappa_2 \quad \kappa_1 (\mathbf{p}^{-2} \otimes \imath_N)]) 
...
```{r define baseline model variables}
...
```{r, extension model posterior distribution, echo = TRUE}
#| code-fold: true
#| code-summary: "See R code"
...
#| tbl-cap: Mean of the Matrix B[0]
knitr::kable(apply(B0.mean, 1:2, mean))
```

```{r random walk extension Matrix B+}
#| label: table - RW Extension B+
#| tbl-cap: Mean of the Matrix B[+]
knitr::kable(apply(B1.mean, 1:2, mean))
```
...
```{r random walk extension lambda}
#| label: table - RW Extension lambda
#| tbl-cap: Mean of the lambda
knitr::kable(mean(lambda.posterior))
```
